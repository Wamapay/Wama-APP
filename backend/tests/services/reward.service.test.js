...
describe("reward.service.evaluateReferralRewards — platform rule 'Reward Test'", () => {
  ...
  it("returns an empty list without querying anything when the admin has disabled 'rewards'", async () => {
    mockPrisma.platformSettings.upsert.mockResolvedValueOnce({
      id: "platform-settings-singleton",
      rewardTiers: [{ milestone: 10, amount: "50" }],
      featuresEnabled: { courses: true, referrals: true, cashback: true, rewards: false, withdrawals: true, reviews: true },
    });
