...
describe("review.service", () => {
  ...
  describe("upsertReview — feature toggle", () => {
    it("refuses new/updated reviews when the admin has disabled the 'reviews' feature", async () => {
      mockPrisma.platformSettings.upsert.mockResolvedValueOnce({
        id: "platform-settings-singleton",
        featuresEnabled: { courses: true, referrals: true, cashback: true, rewards: true, withdrawals: true, reviews: false },
      });

      await expect(
        reviewService.upsertReview("user_1", "course_1", { rating: 5, title: "Great", comment: "Loved it" })
      ).rejects.toMatchObject({ statusCode: 403 });
