...
describe("order.service", () => {
  ...
  describe("createOrder — feature toggle", () => {
    it("Maintenance Mode: blocks all new purchases", async () => {
      mockPrisma.platformSettings.upsert.mockResolvedValueOnce({
        id: "platform-settings-singleton",
        maintenanceMode: true,
      });

      await expect(orderService.createOrder({ userId: "u1", courseId: "c1" })).rejects.toMatchObject({ statusCode: 403 });
      expect(mockPrisma.course.findUnique).not.toHaveBeenCalled();
    });

    it("refuses new purchases when the admin has disabled the 'courses' feature", async () => {
      // mockResolvedValue (not Once) — createOrder now calls getSettings()
      // twice (once for the maintenance-mode check, once for the
      // courses-feature check), and both need the same real values.
      mockPrisma.platformSettings.upsert.mockResolvedValue({
        id: "platform-settings-singleton",
        maintenanceMode: false,
        featuresEnabled: { courses: false, referrals: true, cashback: true, rewards: true, withdrawals: true, reviews: true },
      });
  ...
  describe("listRecentPurchaseActivity (dashboard billboard — replaces the old fake, static list)", () => {
    it("returns an empty list on a brand-new platform with no real purchases yet — never fabricates activity", async () => {
      mockPrisma.order.findMany.mockResolvedValue([]);
      const result = await orderService.listRecentPurchaseActivity();
      expect(result).toEqual([]);
      // Never even bothers querying transactions when there are no orders.
      expect(mockPrisma.transaction.findMany).not.toHaveBeenCalled();
    });

    it("only ever includes PAID orders, ordered most-recent-first", async () => {
      mockPrisma.order.findMany.mockResolvedValue([]);
      await orderService.listRecentPurchaseActivity();
      expect(mockPrisma.order.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { status: "PAID" }, orderBy: { paidAt: "desc" } })
      );
    });

    it("shows a privacy-safe name — first name + last initial only, never the full name", async () => {
      mockPrisma.order.findMany.mockResolvedValue([
        { id: "order_1", paidAt: new Date(), createdAt: new Date(), user: { fullName: "Ama Serwaa" }, course: { title: "Forex Masterclass" } },
      ]);
      mockPrisma.transaction.findMany.mockResolvedValue([]);

      const result = await orderService.listRecentPurchaseActivity();

      expect(result[0].buyerName).toBe("Ama S.");
      expect(JSON.stringify(result)).not.toContain("Serwaa"); // last name never leaks
    });

    it("handles a single-word name without throwing", async () => {
      mockPrisma.order.findMany.mockResolvedValue([
        { id: "order_1", paidAt: new Date(), createdAt: new Date(), user: { fullName: "Cher" }, course: { title: "Forex Masterclass" } },
      ]);
      mockPrisma.transaction.findMany.mockResolvedValue([]);

      const result = await orderService.listRecentPurchaseActivity();
      expect(result[0].buyerName).toBe("Cher");
    });

    it("shows the REAL cashback amount actually credited for that purchase — never recalculated against today's rate", async () => {
      mockPrisma.order.findMany.mockResolvedValue([
        { id: "order_1", paidAt: new Date(), createdAt: new Date(), user: { fullName: "Kojo Boateng" }, course: { title: "Crypto Basics" } },
      ]);
      // This order's REAL recorded cashback was 100 — even if today's
      // configured rate would compute a different number for the same
      // course price, this historical value is what must be shown.
      mockPrisma.transaction.findMany.mockResolvedValue([
        { referenceId: "order_1", amount: 100 },
      ]);

      const result = await orderService.listRecentPurchaseActivity();
      expect(result[0].cashbackAmount).toBe(100);
    });

    it("shows null cashback (never fabricates a number) if no cashback transaction exists for that order", async () => {
      mockPrisma.order.findMany.mockResolvedValue([
        { id: "order_1", paidAt: new Date(), createdAt: new Date(), user: { fullName: "Efua Mensah" }, course: { title: "E-commerce 101" } },
      ]);
      mockPrisma.transaction.findMany.mockResolvedValue([]);

      const result = await orderService.listRecentPurchaseActivity();
      expect(result[0].cashbackAmount).toBeNull();
    });

    it("respects a custom limit, capped at 20", async () => {
      mockPrisma.order.findMany.mockResolvedValue([]);
      await orderService.listRecentPurchaseActivity("500");
      expect(mockPrisma.order.findMany).toHaveBeenCalledWith(expect.objectContaining({ take: 20 }));
    });
  });
});
