...
describe("platformSettings.service", () => {
  ...
  describe("Platform Identity — name/logo/branding", () => {
    it("getPlatformIdentity returns the 5 identity fields plus maintenance status — never rates/features", async () => {
      const identity = await platformSettings.getPlatformIdentity();
      expect(Object.keys(identity).sort()).toEqual(
        ["logoUrl", "platformName", "supportEmail", "supportPhone", "tagline", "maintenanceMode", "maintenanceMessage"].sort()
      );
      expect(identity.platformName).toBe("Wamton");
      expect(identity.maintenanceMode).toBe(false);
    });

    it("real support contact defaults are set out of the box, not left blank", () => {
      expect(platformSettings.DEFAULTS.supportEmail).toBe("werisealways01@gmail.com");
      expect(platformSettings.DEFAULTS.supportPhone).toBe("+233539071126");
    });
  ...
  describe("Emergency Controls — Maintenance Mode (Part 19)", () => {
    it("rejects a non-boolean maintenanceMode", async () => {
      await expect(platformSettings.updateSettings({ maintenanceMode: "yes" }, "admin_1")).rejects.toMatchObject({ statusCode: 400 });
      expect(mockPrisma.platformSettings.update).not.toHaveBeenCalled();
    });

    it("rejects a maintenanceMessage over 500 characters", async () => {
      await expect(
        platformSettings.updateSettings({ maintenanceMessage: "x".repeat(501) }, "admin_1")
      ).rejects.toMatchObject({ statusCode: 400 });
    });

    it("turns maintenance mode on with a real custom message", async () => {
      mockPrisma.platformSettings.update.mockResolvedValue({ maintenanceMode: true, maintenanceMessage: "Back at 3pm" });
      const result = await platformSettings.updateSettings(
        { maintenanceMode: true, maintenanceMessage: "Back at 3pm" },
        "admin_1"
      );
      const callArgs = mockPrisma.platformSettings.update.mock.calls[0][0];
      expect(callArgs.data.maintenanceMode).toBe(true);
      expect(callArgs.data.maintenanceMessage).toBe("Back at 3pm");
      expect(result.maintenanceMode).toBe(true);
    });

    it("turning maintenance back off is a normal, reversible update — no special ceremony", async () => {
      mockPrisma.platformSettings.update.mockResolvedValue({ maintenanceMode: false });
      await platformSettings.updateSettings({ maintenanceMode: false }, "admin_1");
      const callArgs = mockPrisma.platformSettings.update.mock.calls[0][0];
      expect(callArgs.data.maintenanceMode).toBe(false);
    });

    it("clearing the custom message with null works, distinct from an empty string", async () => {
      mockPrisma.platformSettings.update.mockResolvedValue({ maintenanceMessage: null });
      await platformSettings.updateSettings({ maintenanceMessage: null }, "admin_1");
      const callArgs = mockPrisma.platformSettings.update.mock.calls[0][0];
      expect(callArgs.data.maintenanceMessage).toBeNull();
    });
  });
});
