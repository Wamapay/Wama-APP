...
describe("withdrawal.service", () => {
  ...
  describe("createWithdrawal — platform rule 'Withdrawal Test'", () => {
    it("reserves the balance atomically and creates a PENDING withdrawal + ledger transaction", async () => {
      ...
      mockPrisma.user.findUnique.mockResolvedValue({ id: "user_1", withdrawalPinHash: "hashed:1234", withdrawalPinFailedAttempts: 0, withdrawalPinLockedUntil: null });
    ...
    it("prevents a second concurrent withdrawal from over-spending an already-reserved balance", async () => {
      ...
      await expect(
        withdrawalService.createWithdrawal({ userId: "user_1", amount: 60, balanceType: "CASHBACK", paymentMethod: "mobile_money", paymentDetails: SAMPLE_MOBILE_MONEY_DETAILS, pin: "1234" })
      ).rejects.toMatchObject({ statusCode: 400 });
    });
  });
  ...
  describe("createWithdrawal — feature toggle", () => {
    it("refuses new withdrawal requests when the admin has disabled the 'withdrawals' feature", async () => {
      mockPrisma.platformSettings.upsert.mockResolvedValueOnce({
        id: "platform-settings-singleton",
        featuresEnabled: { courses: true, referrals: true, cashback: true, rewards: true, withdrawals: false, reviews: true },
      });

      await expect(
        withdrawalService.createWithdrawal({ userId: "u1", amount: "100", balanceType: "CASHBACK", paymentMethod: "mobile_money", paymentDetails: {} })
      ).rejects.toMatchObject({ statusCode: 403 });
      // The withdrawal-specific lookup (reserving balance etc.) never
      // runs — only the one small per-user-override check happens
      // first (isFeatureEnabled always checks that before the global
      // value), which finds no override here and correctly falls
      // through to the global "disabled" result.
      expect(mockPrisma.user.findUnique).toHaveBeenCalledTimes(1);
      expect(mockPrisma.user.findUnique).toHaveBeenCalledWith({ where: { id: "u1" }, select: { featureOverrides: true } });
    });
  });
});
