...
describe("auth.service", () => {
  ...
  describe("register", () => {
    it("Maintenance Mode: blocks new sign-ups entirely", async () => {
      ...
      await expect(
        authService.register({ fullName: "Jane", email: "jane@example.com", phone: "+233240000001", password: "password123" })
  ...
  describe("login", () => {
    ...
    it("Maintenance Mode: blocks a regular USER from logging in", async () => {
      mockPrisma.platformSettings.upsert.mockResolvedValueOnce({
        id: "platform-settings-singleton",
        maintenanceMode: true,
      });
      mockPrisma.user.findUnique.mockResolvedValue({
        id: "user_1", role: "USER", passwordHash: "hashed:correct-password", status: "ACTIVE", emailVerified: true,
      });
    ...
    it("Maintenance Mode: ADMIN can still log in — the core safety guarantee", async () => {
      // No maintenanceMode mock needed here — ADMIN skips the check
      // entirely by design (see auth.service.js), so getSettings() is
      // never even called for this role. Queuing an unused
      // mockResolvedValueOnce here would leak into a later test.
      mockPrisma.user.findUnique.mockResolvedValue({
        id: "admin_1", role: "ADMIN", passwordHash: "hashed:correct-password", status: "ACTIVE", emailVerified: true,
      });
    ...
    it("Maintenance Mode: a specialized admin role (e.g. FINANCE_ADMIN) can also still log in", async () => {
      // Same reasoning — FINANCE_ADMIN is in ADMIN_ROLES, so this also
      // never calls getSettings().
      mockPrisma.user.findUnique.mockResolvedValue({
        id: "fin_1", role: "FINANCE_ADMIN", passwordHash: "hashed:correct-password", status: "ACTIVE", emailVerified: true,
      });

      const result = await authService.login({ email: "finance@example.com", password: "correct-password" });
      expect(result.tokens).toBeDefined();
    });

    it("does NOT reveal maintenance status for wrong credentials — the generic error still wins", async () => {
      // No maintenanceMode mock needed — the wrong password throws
      // before the maintenance check ever runs, so getSettings() is
      // never called in this path either.
      mockPrisma.user.findUnique.mockResolvedValue({
        id: "user_1", role: "USER", passwordHash: "hashed:correct-password", status: "ACTIVE", emailVerified: true,
      });
    ...
    it("issues a token pair and updates lastLoginAt on success", async () => {
      ...
      expect(mockPrisma.user.update).toHaveBeenCalledWith({
        where: { id: "user_1" },
        data: { lastLoginAt: expect.any(Date), deactivatedAt: null },
      });
    ...
    it("REAL SELF-SERVICE REACTIVATION: logging in always clears deactivatedAt, even if it was already null", async () => {
      mockPrisma.user.findUnique.mockResolvedValue({
        id: "user_1",
        role: "USER",
        passwordHash: "hashed:correct-password",
        status: "ACTIVE",
        emailVerified: true,
        deactivatedAt: new Date(), // this account was self-deactivated
      });
      mockPrisma.user.update.mockResolvedValue({});
      mockPrisma.refreshToken.create.mockResolvedValue({});

      await authService.login({ email: "jane@example.com", password: "correct-password" });
