...
const listMyOrders = asyncHandler(async (req, res) => {  const { items, total, page, pageSize } = await orderService.listOrdersForUser(req.user.id, req.query);
  return ApiResponse.success(res, {
...
const listRecentActivity = asyncHandler(async (req, res) => {
  const activity = await orderService.listRecentPurchaseActivity(req.query.limit);
  return ApiResponse.success(res, { message: "Recent activity retrieved", data: { activity } });
});

module.exports = { createOrder, listMyOrders, getMyOrder, adminListOrders, adminGetOrder, listRecentActivity };
