...
async function createOrder({ userId, courseId, referralCode }) {
  ...
  if (settings.maintenanceMode) {
    throw ApiError.forbidden("The platform is currently under maintenance. Purchases are temporarily unavailable — please check back shortly.");
  }
  if (!(await platformSettings.isFeatureEnabled("courses", userId))) {
    throw ApiError.forbidden("Course purchases are temporarily unavailable. Please try again later.");
  }
...
/**
 * Real "recent activity" for the dashboard billboard — replaces what
 * was previously an entirely fake, hardcoded list of names/times that
 * never changed. Privacy-safe: only "FirstName L." is ever shown, never
 * a full name, and never anything about WHO specifically is buying
 * beyond that. The cashback amount shown is the REAL amount that was
 * actually credited for that purchase (looked up from the real
 * CASHBACK transaction tied to the order) — never recalculated against
 * today's rate, since the rate may have changed since that purchase
 * happened and showing today's rate against an old sale would be
 * inaccurate, not just imprecise.
 */
async function listRecentPurchaseActivity(limit = 10) {
  ...
  const cashbackByOrderId = new Map(cashbackTxns.map((t) => [t.referenceId, t.amount]));

  return orders.map((order) => {
    const nameParts = (order.user.fullName || "Someone").trim().split(/\s+/);
    const privacySafeName =
      nameParts.length > 1 ? `${nameParts[0]} ${nameParts[nameParts.length - 1][0]}.` : nameParts[0];

    return {
      buyerName: privacySafeName,
      courseTitle: order.course.title,
      cashbackAmount: cashbackByOrderId.get(order.id) || null,
