...
async function awardCashback(tx, order) {
  if (!(await platformSettings.isFeatureEnabled("cashback", order.userId))) {
    return { transaction: null, created: false, amount: null };
  }

  const { cashbackRatePercent } = await platformSettings.getSettings();
  const rate = toDecimal(cashbackRatePercent).dividedBy(100);
  ...
  const { transaction, created } = await ledger.recordTransaction(tx, {
    ...
    description: `Cashback (${cashbackRatePercent}%) for order ${order.orderNumber}`,
    applyBalance: true,
  });
...
module.exports = { awardCashback, listCashbackHistory };
