...
async function evaluateReferralRewards(agentId, referenceDate = new Date()) {
  if (!(await platformSettings.isFeatureEnabled("rewards"))) {
    return [];
  }
  ...
  if (!(await platformSettings.isFeatureEnabled("rewards", agent.userId))) {
    return [];
  }
  ...
  const { rewardTiers } = await platformSettings.getSettings();
  const eligibleTiers = rewardTiers.filter((tier) => successfulCount >= tier.milestone);
