 ...
 *
 * Every notification sent here also triggers a real Web Push (see
 * webPush.service.js) to any device the recipient has subscribed on —
 * best-effort: a push failure never affects the real, already-working
...
// Fire real pushes for a batch of recipients — deliberately never
// awaited by the caller's critical path; a slow or failing push
// service must never delay or break the actual notification response.
function firePushToRecipients(recipientIds, title, message) {
  Promise.allSettled(
    recipientIds.map((userId) => webPushService.sendPushToUser(userId, { title, body: message }))
  ).catch(() => {});
}
...
async function sendFromAdmin({ adminId, title, message, audience, userId }) {
  if (audience === "USER") {
    ...
    firePushToRecipients([userId], title, message);
    return { count: 1 };
  }
  ...
  firePushToRecipients(recipientIds, title, message);
