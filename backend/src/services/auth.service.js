...
async function register({ fullName, email, phone, password, referralCode }) {
  // Maintenance Mode (Emergency Controls, Part 19) — nobody registers
  // as an admin-tier account through the public signup form, so this
  // blocks unconditionally, no role check needed here.
  const settings = await platformSettings.getSettings();
  if (settings.maintenanceMode) {
    throw ApiError.forbidden(settings.maintenanceMessage || "The platform is currently under maintenance. New sign-ups are temporarily unavailable — please check back shortly.");
  }
...
async function login({ email, password }) {
  ...
  // Maintenance Mode (Emergency Controls, Part 19) — admin-tier roles
  // always keep working here, specifically so there is never a way to
  // accidentally lock yourself out of turning maintenance back off.
  // Runs after the password check (never reveals maintenance status to
  // someone with wrong credentials) and before verification/tokens.
  if (!ADMIN_ROLES.includes(user.role)) {
    const settings = await platformSettings.getSettings();
    if (settings.maintenanceMode) {
      throw ApiError.forbidden(settings.maintenanceMessage || "The platform is currently under maintenance. Please check back shortly.");
    }
  }
  ...
  await prisma.user.update({
    ...
    data: {
      ...
      // Real self-service reactivation: logging back in is literally
      // what un-deactivates the account (see user.service.js
      // deactivateAccount). A no-op write when deactivatedAt was
      // already null.
      deactivatedAt: null,
    },
  });
