...
async function sendDataExportEmail({ to, fullName, csv }) {
  ...
  const safeName = escapeHtml(fullName || "there");
  const html = `<p>Hi ${safeName},</p><p>Attached is a real export of your Learn &amp; Earn account data — your profile details, orders, and financial transaction history.</p><p>If you didn't request this, you can safely ignore this email.</p>`;
  const text = `Hi ${fullName || "there"},\n\nAttached is a real export of your Learn & Earn account data — your profile details, orders, and financial transaction history.\n\nIf you didn't request this, you can safely ignore this email.`;

  try {
    const { error } = await resend.emails.send({
      from: config.email.from,
      to,
      subject: "Your Learn & Earn data export",
      html,
      text,
      attachments: [{ filename: "learn-and-earn-data-export.csv", content: Buffer.from(csv, "utf-8") }],
    });
    if (error) {
