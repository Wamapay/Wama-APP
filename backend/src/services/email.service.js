...
async function sendVerificationEmail({ to, fullName, verificationUrl }) {
  ...
  const html = `
    <div style="font-family: Arial, Helvetica, sans-serif; max-width: 480px; margin: 0 auto; color: #101828;">
      <h2 style="margin: 0 0 16px;">Wamton</h2>
      <p>Hi ${safeName},</p>
  ...
  const text =
    `Hi ${fullName || "there"},\n\n` +
    `Thanks for creating a Wamton account. Verify your email to activate it:\n` +
    `${verificationUrl}\n\n` +
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      to,
      subject: "Verify your email — Wamton",
      html,
      text,
    });
    if (error) {
...
/**
 * Sends a user's real data export as a CSV attachment. Same
 * fail-quietly-and-report-back pattern as sendVerificationEmail — the
 * caller decides what `sent: false` means for its own response.
 */
async function sendDataExportEmail({ to, fullName, csv }) {
  const resend = getClient();
  if (!resend) {
    logger.error("[email] RESEND_API_KEY is not configured — cannot send data export email.");
    return { sent: false };
  }
  if (!config.email.from) {
    logger.error("[email] EMAIL_FROM is not configured — cannot send data export email.");
    return { sent: false };
  }

  const safeName = escapeHtml(fullName || "there");
  const html = `<p>Hi ${safeName},</p><p>Attached is a real export of your Wamton account data — your profile details, orders, and financial transaction history.</p><p>If you didn't request this, you can safely ignore this email.</p>`;
  const text = `Hi ${fullName || "there"},\n\nAttached is a real export of your Wamton account data — your profile details, orders, and financial transaction history.\n\nIf you didn't request this, you can safely ignore this email.`;

  try {
    const { error } = await resend.emails.send({
      from: config.email.from,
      to,
      subject: "Your Wamton data export",
      html,
      text,
      attachments: [{ filename: "wamton-data-export.csv", content: Buffer.from(csv, "utf-8") }],
    });
    if (error) {
      logger.error(`[email] Resend rejected the data export email: ${error.name || "error"} — ${error.message || "unknown"}`);
      return { sent: false };
    }
    return { sent: true };
  } catch (err) {
    logger.error(`[email] Unexpected error sending data export email: ${err.message}`);
    return { sent: false };
  }
}

module.exports = { sendVerificationEmail, sendDataExportEmail };
