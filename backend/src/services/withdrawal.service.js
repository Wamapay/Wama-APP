...
const withdrawalPinService = require("./withdrawalPin.service");
 ...
 */
async function createWithdrawal({ userId, amount, balanceType, paymentMethod, paymentDetails, pin }) {
  ...
  // Withdrawal PIN — the final security check, separate from the
  // account password. Checked before any balance logic runs, so a
  // wrong PIN never reveals anything about the account's balance.
  await withdrawalPinService.verifyWithdrawalPin(userId, pin);
