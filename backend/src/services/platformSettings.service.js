...
const DEFAULTS = {
  ...
  logoUrl: null,
  tagline: null,
  supportEmail: "werisealways01@gmail.com",
  supportPhone: "+233539071126",
  // Emergency Controls (Part 19) — see schema.prisma's field comment
  // for the full "why" of this design. Off by default, obviously.
...
async function updateSettings(patch, adminId) {
  ...
  if (patch.maintenanceMode !== undefined) {
    if (typeof patch.maintenanceMode !== "boolean") {
      throw ApiError.badRequest("maintenanceMode must be true or false.");
    }
    data.maintenanceMode = patch.maintenanceMode;
  }

  if (patch.maintenanceMessage !== undefined) {
    if (patch.maintenanceMessage !== null && (typeof patch.maintenanceMessage !== "string" || patch.maintenanceMessage.length > 500)) {
      throw ApiError.badRequest("maintenanceMessage must be a string (max 500 characters) or null to clear it.");
    }
...
async function getPlatformIdentity() {
  ...
  // Deliberately included here (not just an admin-only field) — the
  // frontend needs to know maintenance status WITHOUT being logged in,
  // so it can show a real "under maintenance" page instead of a
  // confusing login error.
  identity.maintenanceMode = settings.maintenanceMode;
  identity.maintenanceMessage = settings.maintenanceMessage;
  return identity;
}
