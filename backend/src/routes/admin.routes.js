...
const { messageIdParamSchema } = require("../validators/communityChat.validator");
const { createFeaturedEntrySchema, entryIdParamSchema } = require("../validators/billboard.validator");
...
/** @route POST /api/v1/admin/uploads — real file upload (images/PDF only, never video) */
...
/** @route DELETE /api/v1/admin/community/chat/messages/:id */
router.delete(
  "/community/chat/messages/:id",
  requirePermission(PERMISSIONS.MODERATE_COMMUNITY_CHAT),
  validate(messageIdParamSchema),
  communityChatController.deleteMessage
);

// --- Dashboard activity billboard -------------------------------------------

/** @route GET /api/v1/admin/billboard/featured — admin management view (includes expired entries) */
router.get("/billboard/featured", requirePermission(PERMISSIONS.MANAGE_BILLBOARD), billboardController.listFeaturedEntries);

/** @route POST /api/v1/admin/billboard/featured */
router.post(
  "/billboard/featured",
  requirePermission(PERMISSIONS.MANAGE_BILLBOARD),
  validate(createFeaturedEntrySchema),
  billboardController.createFeaturedEntry
);

/** @route DELETE /api/v1/admin/billboard/featured/:id */
router.delete(
  "/billboard/featured/:id",
  requirePermission(PERMISSIONS.MANAGE_BILLBOARD),
  validate(entryIdParamSchema),
  billboardController.deleteFeaturedEntry
);
