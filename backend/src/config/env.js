...
const config = {
  ...
  // Real file storage for images/PDFs (course thumbnails, lesson PDF
  // materials, platform logo) — see src/services/upload.service.js.
  // Deliberately NOT used for video: video stays a pasted link
  // (YouTube/Vimeo/etc.) rather than a real upload, since video
  // storage/bandwidth costs scale with usage in a way images/PDFs don't.
  cloudinary: {
  ...
  // Optional — shared rate-limit counting across multiple replicas (see
  // src/middleware/rateLimiter.js). Railway's own Redis add-on sets
  // REDIS_URL automatically once provisioned. When unset, rate limiting
  // falls back to the original in-memory behavior (correct for a single
  // instance, just not shared once you run more than one replica).
  redis: {
    url: process.env.REDIS_URL || "",
  },

  // Real Web Push — a free, open web standard (no paid SMS/push vendor
  // needed). Generate a real key pair once with:
  //   node -e "console.log(require('web-push').generateVAPIDKeys())"
  // and set both values below. Optional: without these, push simply
  // isn't sent (see webPush.service.js) — everything else keeps working.
  vapid: {
    publicKey: process.env.VAPID_PUBLIC_KEY || "",
    privateKey: process.env.VAPID_PRIVATE_KEY || "",
    subject: process.env.VAPID_SUBJECT || "mailto:hello@wamton.com",
  },
